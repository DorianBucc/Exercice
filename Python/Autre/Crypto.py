...
def Cesar(text,clef:int): 
    ...
    for i in text:
        ...
        y = chr(r+65)
        chainechiffre += y
    print("Chaîne chiffrée :" + chainechiffre)

def DeCesar(text,clef:int): 
    chainechiffre = ""
    for i in text:
        r = ord(i)-65
        r -= clef
        ...
        y = chr(r+65)
        chainechiffre += y
    print("Chaîne déchiffrée :" + chainechiffre)

# Cesar("BONJOUR",3)
# DeCesar("ERQMRXU",3)

def Cesar2(text,clefa:int,clefb:int): 
    chainechiffre = ""
    for i in text:
        r = ord(i)-65
        r = clefa * r + clefb
...
def DeCesar2(text,clefa:int,clefb:int,Z:int): 
    x = affine_inverse(clefa,clefb,Z)
    a = x[0]
    b = x[1]
    chainechiffre = ""
    for i in text:
        ...
        chainechiffre += y
    print("Chaîne déchiffrée :" + chainechiffre)
# Cesar2("BONJOUR",17,12)
# DeCesar2("DQZJQOP",17,12,26)
...
def DeVigenere(str,key):
	...
	for i in str:
		...
		cout = cout + 1
	return rtr
# print(Vigenere("BONJOUR",4))
# print(DeVigenere("BPPMOVT",4))
...
def Permutation(message,tab):
	rtr = ""
	for i in message:
		rtr += tab[ord(i)-65]
	return rtr

def DePermutation(message,tab:list):
	rtr = ""
	for i in message:
		rtr += chr(tab.index(i) + 65)
	return rtr

def DePermutation2(message,tab):
	rtr = ""
	for i in message:
		cout = 0
		for y in tab:
...
#tab = ["V","H","N","E","J","D","K","P","T","A","Q","W","U","Y","F","S","G","L","M","B","C","X","Z","R","I","O"]
#print(Permutation("BONJOURMADAME",tab))
#print(DePermutation("HFYAFCLUVEVUJ",tab))
